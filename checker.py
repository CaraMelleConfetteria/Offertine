...
# Ricostruisce il PEM se i newline sono stati persi nel Secret
def fix_pem(key):
    key = key.strip()
    if '\\n' in key:
        key = key.replace('\\n', '\n')
    if '\n' not in key:
        # Ricostruisci il formato PEM
        key = key.replace('-----BEGIN EC PRIVATE KEY-----', '').replace('-----END EC PRIVATE KEY-----', '').strip()
